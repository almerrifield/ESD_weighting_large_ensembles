...
DATAPATH = '/net/h2o/climphys/meranna/model_weighting_output/CMIP5/'
PLOTPATH = '../../plots/'
    ...
    if varn == 'tas':
        return (
            dict(
                ...
                vmax=.8,
                cmap='RdBu_r',
                levels=np.concatenate((np.arange(-.8, 0, .2), np.arange(.2, 0.8, .2))),
                extend='both',
            ),
...
def preprocess(ds, varn, relative=False):
    with warnings.catch_warnings():
        ...
        av = ds[varn].mean('model_ensemble')  # unweighted mean
        avw = xr.apply_ufunc(
            np.average, ds[varn],
            input_core_dims=[['model_ensemble']],
            kwargs={'weights': ds['weights']},
            vectorize=True)  # weighted mean
    av_diff = avw - av
    if relative:
        av_diff /= 0.01 * av
    ds['mean'] = av
...
def bootstrap(ds, varn, percentile=(.05, .95), relative=False):
    diffs = []
    weights = ds['weights'].copy().data # shuffle weights
    for idx in range(1000):
        ...
        with warnings.catch_warnings():
            ...
            avw = xr.apply_ufunc(
                ...
                vectorize=True)
        av_diff = avw - ds['mean'] # subtract bootstrapped weighted mean from mean
        if relative:
